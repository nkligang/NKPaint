...
BOOL _AFX_MOUSEANCHORWND::Create(CADMMScrollView* pParent)
{
	...
	else
	{
		...
		TRACE("Failed to create mouse anchor window!");
	}
#endif
...
void CADMMScrollView::OnPrepareDC(CDC* pDC, CPrintInfo* pInfo)
{
	...
	{
		TRACE("Error: must call SetScrollSizes() or SetScaleToFitSize()");
		TRACE("\tbefore painting scroll view.\n");
		ASSERT(FALSE);
		return;
	}
#endif //_DEBUG
	...
	case MM_SCALETOFIT:
		...
		if (m_totalDev.cx == 0 || m_totalDev.cy == 0)
			TRACE("Warning: CADMMScrollView scaled to nothing.\n");
		break;
...
{
	if (m_nMapMode <= 0)
		TRACE("Warning: CADMMScrollView::GetDeviceScrollSizes returning invalid mapping mode.\n");
	nMapMode = m_nMapMode;
